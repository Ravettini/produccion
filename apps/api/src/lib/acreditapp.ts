...
export type AcreditappAttendanceStats = {
  id: string;
  name?: string;
  convocados: number;
  asistidos: number;
...
/** Extrae el id de evento de Acreditapp desde el link guardado. */
export function extractAcreditappEventId(link: string | null | undefined): string | null {
  if (!link?.trim()) return null;
  try {
    const url = new URL(link.trim());
    const m = url.pathname.match(/\/events\/([^/]+)/i);
    return m?.[1] ? decodeURIComponent(m[1]) : null;
  } catch {
    const m = String(link).match(/\/events\/([^/?#]+)/i);
    return m?.[1] ? decodeURIComponent(m[1]) : null;
  }
}

/**
 * Consulta convocados (total personas) y asistidos (acreditados) en Acreditapp.
 */
export async function fetchAcreditappAttendance(
  eventIdOrLink: string
): Promise<AcreditappAttendanceStats> {
  const { apiBase, apiKey } = getAcreditappConfig();
  const eventId = extractAcreditappEventId(eventIdOrLink) ?? eventIdOrLink.trim();
  if (!eventId) {
    throw new AcreditappError("No se pudo determinar el id del evento en Acreditapp");
  }
  ...
  try {
    const res = await fetch(`${apiBase}/api/v1/external/events/${encodeURIComponent(eventId)}/stats`, {
      method: "GET",
      headers: {
      ...
      const detail =
        typeof data.error === "string"
          ...
          : typeof data.message === "string"
            ...
            : `HTTP ${res.status}`;
      throw new AcreditappError(
        `No se pudieron obtener asistentes de Acreditapp (${detail})`,
        res.status
      );
    }

    const convocadosRaw = data.convocados ?? data.total ?? data.totalBase;
    const asistidosRaw = data.asistidos ?? data.accredited;
    const convocados = Number(convocadosRaw);
    const asistidos = Number(asistidosRaw);
    if (!Number.isFinite(convocados) || !Number.isFinite(asistidos)) {
      throw new AcreditappError("Acreditapp devolvió estadísticas inválidas");
    }

    return {
      id: typeof data.id === "string" ? data.id : eventId,
      name: typeof data.name === "string" ? data.name : undefined,
      convocados: Math.max(0, Math.trunc(convocados)),
      asistidos: Math.max(0, Math.trunc(asistidos)),
    };
  } catch (err) {
    if (err instanceof AcreditappError) throw err;
    if (err instanceof Error && err.name === "AbortError") {
      throw new AcreditappError("Timeout al consultar asistentes en Acreditapp");
    }
    const msg = err instanceof Error ? err.message : String(err);
    throw new AcreditappError(`Error de red al consultar Acreditapp: ${msg}`);
  } finally {
    clearTimeout(timer);
  }
}
